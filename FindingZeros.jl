module Finding_Zeros
    ...
    y_values = [0, 1, 4, 9, 16]
    target_y = 8
    iterated_inverse_interpolation(x_values, y_values; target_y) # Output 2.622222222222222
    ```
    """
    function iterated_inverse_interpolation(x_values, y_values; target_y = 0)
        n = length(x_values)
        Q = map(Float64, x_values) # Initialize the first column of the table with y-values
        for j in 2:n
            for i in 1:n-j+1
